...
class Signatures(Base):
    ...
    def create(
        ...
        *,
        ...
        """Opens a ceremony. Returns the row with a freshly generated
        sign_id. Does NOT flush the sign URL; the caller (create handler)
        builds that from `sign_id` + the microfront base URL.
        """
        now = _utcnow()
        row = cls(
            sign_id=new_sign_id(),
